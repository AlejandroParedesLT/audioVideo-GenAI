...
class TrainLoop:
    def __init__(
        ...
        *,
        ...
        if isinstance(model, th.nn.DataParallel) or isinstance(model, th.nn.parallel.DistributedDataParallel):
        ...
        signal.signal(signal.SIGUSR1, self.handle_exit)  # Some SLURM setups send SIGUSR1 instead
        ...
        #             # First, check if the model contains an attribute 'module'
        # if isinstance(self.model, th.nn.DataParallel) or isinstance(self.model, th.nn.parallel.DistributedDataParallel):                
    ...
    def save(self):
        def save_checkpoint(rate, params):
            # First, check if the model contains an attribute 'module'
            if hasattr(self.mp_trainer, 'module'):
                if isinstance(self.mp_trainer.module, th.nn.DataParallel):
                    state_dict = self.mp_trainer.module.master_params_to_state_dict(params)
                elif isinstance(self.mp_trainer.module, th.nn.parallel.DistributedDataParallel):
                    state_dict = self.mp_trainer.module.master_params_to_state_dict(params)
                else:
                    state_dict = self.mp_trainer.master_params_to_state_dict(params)
            else:
                # Handle the case where 'module' doesn't exist on self.mp_trainer
                state_dict = self.mp_trainer.master_params_to_state_dict(params)
    ...
    def handle_exit(self, signum, frame):
        if dist.get_rank() == 0:
            logger.log(f"Received signal {signum}. Saving checkpoint before exit.")
        self.save()
        if dist.get_rank() == 0:
            logger.log("Checkpoint saved. Exiting now.")
        dist.barrier()  # Ensure all ranks wait before exit
        os._exit(0)  # Immediate exit without traceback
