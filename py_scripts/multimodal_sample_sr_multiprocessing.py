...
def main():
    ...
    #     )
    #     print('Is able to load the model')
    #     sr_model.to(dist_util.dev())
    #     print('Loads model to device')
    #     if args.use_fp16:
    ...
    for model_path in multimodal_name_list:
        print('Model path for the state dict', model_path)
        checkpoint = dist_util.load_state_dict(model_path, map_location="cpu")
        # Remove the 'module.' prefix if it exists in the checkpoint keys
        new_state_dict={}
        ...
        # )
        multimodal_model.load_state_dict_(
            new_state_dict, is_strict=args.is_strict
        )
...
def create_argparser():
    ...
    print('Executing model_diffusion default')
    defaults.update(model_and_diffusion_defaults())
    print('Executing image sr')
